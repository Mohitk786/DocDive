...
const Dashboard = ({subscriptionPlan}:PageProps) =>{
    ...
    const {mutate: deleteFile} = trpc.deleteFile.useMutation({
        onSuccess: () => {
            utils.getUserFiles.invalidate(); //on successfully removing item from DB we are invalidating all files again to update on screen
        }, 
        onMutate: ({id})=>{
            ...
            {/* display all user files */}
            {
                files && files?.length !== 0 ? (
                                ...
                                className="col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow transition hover:shadow-lg">
                                    <Link href={`/dashboard/${file.id}`} className="flex flex-col gap-2">
                                        <div className="pt-6 px-6 flex w-full items-center justify-between space-x-6">
                                            <div className="h-10 w-10 flex-shrink-0 bg-gradient-to-r from-cyan-500 to-blue-500"/>                                        
                                    ...
                                    <div className="px-6 mt-4 grid grid-cols-3 place-items-center py-2 gap-6 text-xs text-zinc-500">
                                        ...
                                        {/* delete file */}
                                        <Button 
                                            onClick={()=>deleteFile({id: file.id})}  //file-id is passed as input to api
                                            className="w-full" 
                                            size="sm" 
                                            variant='destructive'>
                    ...
                    <div className="mt-16 flex flex-col items-center gap-2">
                        <Ghost className="h-8 w-8 text-zinc-8000" />
